...
import org.exoplatform.contact.webui.SelectItem;
import org.exoplatform.contact.webui.SelectOption;
import org.exoplatform.contact.webui.SelectOptionGroup;
import org.exoplatform.contact.webui.Selector;
import org.exoplatform.contact.webui.UIFormSelectBoxWithGroups;
...
public class UIEventForm extends UIFormTabPane implements UIPopupComponent, Selector{
  ...
  public static List<SelectItemOption<String>> getCategory() throws Exception {
    ...
    CalendarService calendarService = CalendarUtils.getCalendarService() ;
    List<EventCategory> eventCategories = calendarService.getSysEventCategories(Util.getPortalRequestContext().getRemoteUser()) ;
    ...
    public void execute(Event<UIEventForm> event) throws Exception {
      ...
      if(!uiForm.isEventDetailValid()) {
        ...
        try {
          ...
          if(uiForm.calType_.equals(CalendarUtils.PRIVATE_TYPE)) {
            CalendarUtils.getCalendarService().saveUserEventSys(username, calendarId, calendarEvent, uiForm.isAddNew_) ;
          }else if(uiForm.calType_.equals(CalendarUtils.SHARED_TYPE)){
