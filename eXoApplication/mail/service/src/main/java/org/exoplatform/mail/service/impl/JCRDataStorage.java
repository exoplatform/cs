...
public class JCRDataStorage implements DataStorage {
  ...
  public void delegateAccount(String username , String reciver, String accountId) throws Exception {
    ...
    try {
      ...
      if(logger.isDebugEnabled()) logger.debug(" Delegate account error :  " + e.getMessage()) ;
  ...
  private Map<String, String> valuesToMap(Value[] val) throws Exception {
    ...
    try {
      ...
      if(logger.isDebugEnabled()) logger.debug("no permission");
    }
    return map;
  }
  ...
  private String[] mapToStrings(Map<String, String> map) throws Exception {
    ...
    try {
      ...
      if(logger.isDebugEnabled()) logger.debug("permission empty");
    }
  ...
  public void removeMessage(String username, String accountId, Message message) throws Exception {
    ...
    try {
      sProvider = createSessionProvider();
      Node msgStoreNode = getDateStoreNode(sProvider,
                                           username,
                                           accountId,
    ...
    try {
      ...
      try {
        if (currentFolderNode != null)
          currentFolderNode.setProperty(Utils.EXO_UNREADMESSAGES,
                                        (currentFolderNode.getProperty(Utils.EXO_UNREADMESSAGES)
                                            .getLong() - deUnreadNumber));
        if (destFolderNode != null)
          destFolderNode.setProperty(Utils.EXO_UNREADMESSAGES,
      ...
      try {
        if (currentFolderNode != null)
          currentFolderNode.setProperty(Utils.EXO_TOTALMESSAGE,
                                        (currentFolderNode.getProperty(Utils.EXO_TOTALMESSAGE)
                                            .getLong() - deTotalMessage));
        if (destFolderNode != null)
    ...
    try {
      ...
      if (newAccount != null) {
        ...
        if (account.getLastCheckedDate() != null)
          newAccount.setProperty(Utils.EXO_LAST_CHECKED_TIME, account.getLastCheckedDate()
                                 .getTime());
        else
          newAccount.setProperty(Utils.EXO_LAST_CHECKED_TIME, (Value) null);

        if (account.getLastStartCheckingTime() != null)
          newAccount.setProperty(Utils.EXO_LAST_START_CHECKING_TIME,
                                 account.getLastStartCheckingTime().getTime());
        else
        ...
        newAccount.setProperty(Utils.EXO_SERVERPROPERTIES,
                               values.toArray(new String[account.getServerProperties().size()]));
    ...
    try {
      ...
      try {
        ...
        for (int i = 0; i < folderIds.length; i++) {
          ...
          t1 = System.currentTimeMillis();
          byte checkDuplicate = checkDuplicateStatus(sProvider,
                                                     username,
                                                     msgHomeNode,
                                                     accId,
                                                     msgNode,
      ...
      try {
        ...
        try {
           ...
           * MimeMessageParser.getMD5MsgId(msg);
           */
        ...
        node.setProperty(Utils.EXO_REPLYTO,
                         Utils.decodeText(InternetAddress.toString(msg.getReplyTo())));
        ...
        if (infoObj != null && continuation != null)
          setCometdMessage(continuation, infoObj, from, msgId, isReadMessage, subject, Utils.convertSize(msgSize), 
                           accId, gc, sc, currentUserName, username);
        if (saveTotal)
  ...
  public Folder getFolder(String username, String accountId, String folderId) throws Exception {
    ...
    try {
      ...
      if (node != null) folder = getFolder(node);
    } finally {
      closeSessionProvider(sProvider);
    }
    return folder;
  ...
  public void saveFilter(String username, String accountId, MessageFilter filter, boolean applyAll) throws Exception {
    ...
    try {
      ...
      if (!Utils.isEmptyField(filter.getApplyFolder()))
        ...
        filterNode.setProperty(Utils.EXO_APPLY_FOLDER, Utils.generateFID(accountId,
                                                                         Utils.FD_INBOX,
                                                                         false));
    ...
    try {
      ...
      for (Message msg : msgList) {
        ...
        if (property.equals(Utils.EXO_STAR)) {
          ...
          Node currentFolderNode = getFolderNodeById(sProvider,
                                                     username,
                                                     accountId,
                                                     msgNode.getProperty(Utils.MSG_FOLDERS)
  ...
  public void addMessageToThread(SessionProvider sProvider,
    ...
    try {
      ...
      if (converNodeChilds != null && converNodeChilds.size() > 0) {
        ...
        Node converNodeParent = getMatchingThreadBefore(sProvider,
                                                        username,
                                                        accountId,
                                                        inReplyToHeader,
                                                        msgNode);
  ...
  public boolean savePOP3Message(String username,
    ...
    try {
      ...
      try {
        ...
        logger.debug("Saved body and attachment of message .... size : " + Math.abs(msg.getSize())
                     + " B");
        t2 = System.currentTimeMillis();
        ...
        node.save();
        if (infoObj != null && continuation != null)
          setCometdMessage(continuation, infoObj, from, msgId, isReadMessage, 
                           subject, Utils.convertSize(Math.abs(msg.getSize())), accId, gc, sc, currentUserName, username);
        t4 = System.currentTimeMillis();
        logger.warn("Saved total message to JCR finished : " + (t4 - t1) + " ms");
        logger.debug("Adding message to thread ...");
        t1 = System.currentTimeMillis();
        addMessageToThread(sProvider,
                           username,
                           accId,
                           MimeMessageParser.getInReplyToHeader(msg),
                           node);
        t2 = System.currentTimeMillis();
        logger.debug("Added message to thread finished : " + (t2 - t1) + " ms");
  ...
  private void saveFolderNode(Node folderNode, Folder folder) throws Exception{
    ...
    if (folder.getLastStartCheckingTime() != null)
      folderNode.setProperty(Utils.EXO_LAST_START_CHECKING_TIME, folder.getLastStartCheckingTime()
                           .getTime());
    else
  ...
  private void setCometdMessage(ContinuationService continuation, Info infoObj, String from, String msgId, boolean isReadMessage, String subject, String size, String accId, Calendar gc, Calendar sc, String currentUserName, String username){
    ...
    try {
      ...
      if(logger.isDebugEnabled()) logger.debug("Cannot create json object for cometd", e);
      return;
    }
