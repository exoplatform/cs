...
public class UIComposeForm extends UIForm implements UIPopupComponent, UISelectable {
  ...
  public void init(String accountId, Message msg, int composeType) throws Exception {
    ...
    }else logger.info("The message is null, edit the empty draft message. Or attachment message haven't damaged");
	...
	public void addToUploadFileList(Attachment attachfile) {
		...
		UIComposeInput inputSet = getChildById(FIELD_TO_SET);
		inputSet.addUIFormInput(new UIFormCheckBoxInput<Boolean>(attachfile
				.getId(), null, null).setChecked(true));
	}
	...
	public void fillFields(Message msg) throws Exception {
		...
		MailSetting mailSetting = mailSrv.getMailSetting(MailUtils
				.getCurrentUser());
		UIComposeInput inputSet = getChildById(FIELD_TO_SET);
		String subject = "";
		String replyContent = "";
			  ...
			  try{
				...
				if (attachments_.size() > 0) {
					for (ActionData actionData : getUploadFileList()) {
						inputSet
								.addUIFormInput(new UIFormCheckBoxInput<Boolean>(
										actionData.getActionParameter(), null,
		...
		case MESSAGE_REPLY_ALL:
			...
			MailService mailSvr = this
					.getApplicationComponent(MailService.class);
			Account account = mailSvr.getAccountById(
					MailUtils.getCurrentUser(), accountId_);
			for (int i = 0; i < msgToAdds.length; i++) {
				if (msgToAdds[i] != null
						&& !msgToAdds[i].getAddress().equalsIgnoreCase(
			...
			for (int i = 0; i < msgCcAdds.length; i++) {
				if (msgCcAdds[i] != null
						&& !msgCcAdds[i].getAddress().equalsIgnoreCase(
								account.getEmailAddress())
						&& !msgCcAdds[i].getAddress().equalsIgnoreCase(
								account.getIncomingUser())) {
					if (replyCc.trim().length() > 0)
		...
		case MESSAGE_FOWARD:
			...
			if (!mailSetting.forwardWithAtt()) {
				forwardTxt
						.append("<br><br>-------- Original Message --------<br>");
				forwardTxt.append("Subject: ").append(
						MailUtils.encodeHTML(msg.getSubject())).append("<br>");
				forwardTxt.append("Date: ").append(msg.getSendDate()).append(
						"<br>");
				forwardTxt.append("From: ");

				InternetAddress[] addresses = Utils.getInternetAddress(msg
						.getFrom());
				for (int i = 0; i < addresses.length; i++) {
					...
					if (addresses[i] != null)
						forwardTxt.append(Utils.getPersonal(addresses[i]))
								.append(" \"")
								.append(addresses[i].getAddress()).append("\"");
				}
				forwardTxt.append("<br>To: ");

				InternetAddress[] toAddresses = Utils
						.getInternetAddress(toAddress);
				for (int i = 0; i < toAddresses.length; i++) {
					...
					if (toAddresses[i] != null)
						forwardTxt.append(Utils.getPersonal(toAddresses[i]))
								.append(" \"").append(
										toAddresses[i].getAddress()).append(
										"\"");
				}
			...
			refreshUploadFileList();
			for (ActionData actionData : getUploadFileList()) {
				inputSet.addUIFormInput(new UIFormCheckBoxInput<Boolean>(
						actionData.getActionParameter(), null, null)
						.setChecked(true));
			}
	...
	private String getReplyContent(Message msg) throws Exception {
		...
		if (isVisualEditor) {
			content = "<br><br><div> On "
					+ MailUtils.formatDate("MMM dd, yyyy HH:mm aaa", msg
							.getSendDate(), locale) + ", " + from
					+ " wrote: <br>";
	...
	private String formatContent(Message msg) throws Exception {
		...
		if (isVisualEditor
				&& (msg.getContentType() != null && msg.getContentType()
	...
	private String addOriginalMessageAsAttach(Message msg) throws Exception {
		...
		ByteArrayOutputStream outputStream = (ByteArrayOutputStream) mailSrv
				.exportMessage(username, this.accountId_, msg);
	...
	private Message getNewMessage() throws Exception {
		...
		Account account = mailSvr.getAccountById(usename, this
				.getFieldFromValue());
		String from = account.getUserDisplayName() + "<"
				+ account.getEmailAddress() + ">";
		...
		if (getComposeType() == MESSAGE_REPLY
				|| getComposeType() == MESSAGE_REPLY_ALL
	...
	/**
	 * @param to
	 *            : exited value from to, cc, bcc field
	 * @param fieldToGroup
	 *            : groupId to lookup email
	 * @return String of all email
	 */
	private String addAllMailFromGroup(String to, String fieldToGroup)
			throws Exception {
		StringBuffer mailAddresses = new StringBuffer();
		try {
			Collection<String> groupIDList = getGroupDataValues(fieldToGroup)
					.values();
			...
			if (groupIDList != null && groupIDList.size() > 0) {
				...
				for (String groupId : groupIDList) {
					...
					if (ALL.equals(groupId)) {
						...
						allEmailMap.putAll(contactSrv.searchEmails(MailUtils
								.getCurrentUser(), filter));
					} else if (SHARED.equals(groupId)) {
						ContactFilter filter = new ContactFilter();
						filter.setType(SHARED);
						filter.setSearchSharedContacts(true);
						sharedEmailMap.putAll(contactSrv.searchEmails(MailUtils
								.getCurrentUser(), filter));
					} else {
						ContactFilter filter = new ContactFilter();
						...
						otherEmailMap.putAll(contactSrv.searchEmails(MailUtils
								.getCurrentUser(), filter));
					}
				}
	...
	public boolean fromDrafts() {
		return (getMessage() != null
				&& getMessage().getFolders()[0].equals(Utils.generateFID(
						accountId_, Utils.FD_DRAFTS, false)) || getComposeType() == MESSAGE_IN_DRAFT);
	}
	...
	static public class SendActionListener extends EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			...
			UIMailPortlet uiPortlet = uiComposeForm
					.getAncestorOfType(UIMailPortlet.class);
			MailService mailSvr = uiComposeForm
					.getApplicationComponent(MailService.class);
			...
			if (MailUtils.isFieldEmpty(message.getMessageTo())
				...
				UIApplication uiApp = uiComposeForm
						.getAncestorOfType(UIApplication.class);
				uiApp.addMessage(new ApplicationMessage(
						"UIComposeForm.msg.select-at-least-recipient", null));
			...
			try {
				...
				uiApp
						.addMessage(new ApplicationMessage(
								"UIComposeForm.msg.there-was-an-error-parsing-the-addresses-sending-failed",
								null));
				event.getRequestContext().addUIComponentToUpdateByAjax(
						uiApp.getUIPopupMessages());
				return;
				...
				if (acc.isOutgoingAuthentication()
						&& acc.useIncomingSettingForOutgoingAuthent()) {
					UIPopupActionContainer uiActionContainer = uiComposeForm
							.getAncestorOfType(UIPopupActionContainer.class);
					UIPopupAction uiChildPopup = uiActionContainer
							.getChild(UIPopupAction.class);
					UIEnterPasswordDialog enterPasswordDialog = uiChildPopup
							.createUIComponent(UIEnterPasswordDialog.class,
									null, null);
					...
					uiApp
							.addMessage(new ApplicationMessage(
									"UIComposeForm.msg.please-check-configuration-for-smtp-server",
									null));
					event.getRequestContext().addUIComponentToUpdateByAjax(
							uiApp.getUIPopupMessages());
				}
				...
				if (e.getMessage().contains("Authentication Required")) {
					uiApp
							.addMessage(new ApplicationMessage(
									"UIComposeForm.msg.check-authentication-smtp-outgoingServer",
									null));
				} else {
					uiApp
							.addMessage(new ApplicationMessage(
									"UIComposeForm.msg.sorry-there-was-an-error-sending-the-message-sending-failed",
									null));
				}
				event.getRequestContext().addUIComponentToUpdateByAjax(
						uiApp.getUIPopupMessages());
				...
				uiApp
						.addMessage(new ApplicationMessage(
								"UIComposeForm.msg.there-was-an-unexpected-error-sending-falied",
								null));
				event.getRequestContext().addUIComponentToUpdateByAjax(
						uiApp.getUIPopupMessages());
				return;
			...
			try {
			  Account account = mailSvr.getAccountById(usename, accountId);
				uiComposeForm.saveToSentFolder(usename, account, message);
				UIMessageList uiMessageList = uiPortlet
						.findFirstComponentOfType(UIMessageList.class);
				UIMessagePreview uiMsgPreview = uiPortlet
						.findFirstComponentOfType(UIMessagePreview.class);
				uiMessageList.updateList();
				if (uiMsgPreview != null
						&& uiMsgPreview.getMessage() != null
						&& uiMsgPreview.getMessage().getId().equals(
								message.getId())) {
					uiMsgPreview.setMessage(null);
					event.getRequestContext().addUIComponentToUpdateByAjax(
							uiMsgPreview.getParent());
				} else {
					event
							.getRequestContext()
							.addUIComponentToUpdateByAjax(
									uiPortlet
											.findFirstComponentOfType(UIMessageArea.class));
				}
				event
						.getRequestContext()
						.addUIComponentToUpdateByAjax(
								uiPortlet
										.findFirstComponentOfType(UIFolderContainer.class));
				UIPopupAction uiChildPopup = uiComposeForm
						.getAncestorOfType(UIPopupAction.class);
				uiChildPopup.deActivate();
				event.getRequestContext().addUIComponentToUpdateByAjax(
						uiChildPopup);
				for (Attachment a : uiComposeForm.getAttachFileList()) {
					UIAttachFileForm.removeUploadTemp(uiComposeForm
							.getApplicationComponent(UploadService.class), a
							.getResoureId());
				}
			} catch (Exception e) {
				uiApp.addMessage(new ApplicationMessage(
						"UIComposeForm.msg.save-sent-error", null));
				event.getRequestContext().addUIComponentToUpdateByAjax(
						uiApp.getUIPopupMessages());
				uiComposeForm.getAncestorOfType(UIPopupAction.class)
						.deActivate();
			}
	...
	public void saveToSentFolder(String usename, Account account,
		...
		if (!fromDrafts && isSaved) {
			...
			message.setFolders(new String[] { Utils.generateFID(account.getId(),
					Utils.FD_SENT, false) });
			mailSvr.saveMessage(usename, account, parentPath_, message, true);
		} else if (fromDrafts) {
			Folder drafts = mailSvr.getFolder(usename, account.getId(), Utils
					.generateFID(account.getId(), Utils.FD_DRAFTS, false));
			if (isSaved) {
				message.setFolders(new String[] { Utils.generateFID(account.getId(),
						Utils.FD_SENT, false) });
				mailSvr.saveMessage(usename, account, parentPath_, message,
						false);
			} else {
	...
	static public class SaveDraftActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm composeForm = event.getSource();
			UIMailPortlet uiPortlet = composeForm
					.getAncestorOfType(UIMailPortlet.class);
			UIFolderContainer uiFolderContainer = uiPortlet
					.findFirstComponentOfType(UIFolderContainer.class);
			MailService mailSvr = composeForm
					.getApplicationComponent(MailService.class);
			String accountId = uiPortlet.findFirstComponentOfType(
					UISelectAccount.class).getSelectedValue();
			String usename = uiPortlet.getCurrentUser();

			UIPopupAction uiChildPopup = composeForm
					.getAncestorOfType(UIPopupAction.class);
			...
			message.setReplyTo(message.getMessageTo());
			try {
				String draftFolderId = Utils.generateFID(accountId,
						Utils.FD_DRAFTS, false);
				...
				if (!composeForm.fromDrafts()) {
					mailSvr.saveMessage(usename, mailSvr.getAccountById(usename, accountId),
							composeForm.parentPath_, message, true);
					Folder drafts = mailSvr.getFolder(usename, accountId,
							draftFolderId);
					drafts.setTotalMessage(drafts.getTotalMessage() + 1);
			...
			catch (AuthenticationFailedException e)
			{
			  ...
			  return;
				...
				UIApplication uiApp = composeForm
						.getAncestorOfType(UIApplication.class);
				uiApp.addMessage(new ApplicationMessage(
						"UIComposeForm.msg.save-draft-error", null));
				event.getRequestContext().addUIComponentToUpdateByAjax(
						uiApp.getUIPopupMessages());
			}
			// update ui
			...
			if (selectedFolder != null
					&& selectedFolder.equals(Utils.generateFID(accountId,
							Utils.FD_DRAFTS, false))) {
				UIMessageList uiMsgList = uiPortlet
						.findFirstComponentOfType(UIMessageList.class);
				UIMessagePreview uiMsgPreview = uiPortlet
						.findFirstComponentOfType(UIMessagePreview.class);
				uiMsgList.setMessagePageList(mailSvr.getMessagePageList(
						usename, uiMsgList.getMessageFilter()));
						...
						for (Message msg : showedMsg) {
							if (message.getId().equals(msg.getId())) {
								...
								message = mailSvr.loadTotalMessage(usename,
										accountId, mailSvr.getMessageById(
												usename, accountId, message
														.getId()));
			...
			for (Attachment a : composeForm.getAttachFileList()) {
				UIAttachFileForm.removeUploadTemp(composeForm
						.getApplicationComponent(UploadService.class), a
						.getResoureId());
			}
	...
	static public class DiscardChangeActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiForm = event.getSource();
			uiForm.resetFields();
			for (Attachment a : uiForm.getAttachFileList()) {
				UIAttachFileForm.removeUploadTemp(uiForm
						.getApplicationComponent(UploadService.class), a
						.getResoureId());
			}
			UIPopupAction uiPopupAction = uiForm
					.getAncestorOfType(UIPopupAction.class);
	...
	static public class AttachmentActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiForm = event.getSource();
			UIPopupActionContainer uiActionContainer = uiForm
					.getAncestorOfType(UIPopupActionContainer.class);
			UIPopupAction uiChildPopup = uiActionContainer
					.getChild(UIPopupAction.class);
  ...
  static public class CallDMSSelectorActionListener extends EventListener<UIComposeForm> {
	...
	static public class DownloadActionListener extends
		...
		public void execute(Event<UIComposeForm> event) throws Exception {
			...
			for (Attachment attach : uiComposeForm.getAttachFileList()) {
				if (attach.getId().equals(attId)) {
					DownloadResource dresource = new InputStreamDownloadResource(
							attach.getInputStream(), attach.getMimeType());
					DownloadService dservice = (DownloadService) PortalContainer
							.getInstance().getComponentInstanceOfType(
									DownloadService.class);
					dresource.setDownloadName(attach.getName());
					String downloadLink = dservice.getDownloadLink(dservice
			...
			event.getRequestContext().addUIComponentToUpdateByAjax(
					uiComposeForm.getChildById(FIELD_TO_SET));
		}
	}

	static public class RemoveAttachmentActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiComposeForm = event.getSource();
			...
			while (iter.hasNext()) {
				...
				if (att.getId().equals(attFileId)) {
					UIAttachFileForm.removeUploadTemp(uiComposeForm
							.getApplicationComponent(UploadService.class), att
							.getResoureId());
					iter.remove();
				}
			}
	...
	static public class ToActionListener extends EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			...
			UIPopupAction uiChildPopup = uiActionContainer
					.getChild(UIPopupAction.class);
			UIAddressForm uiAddress = uiChildPopup.activate(
					UIAddressForm.class, 650);
			uiAddress.setRecipientsType(FIELD_TO);
			String toAddressString = uiComposeForm.getFieldToValue();
			InternetAddress[] toAddresses = Utils
					.getInternetAddress(toAddressString);
	...
	static public class ToCCActionListener extends EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			...
			InternetAddress[] ccAddresses = Utils
					.getInternetAddress(ccAddressString);
			List<String> emailList = new ArrayList<String>();
	...
	static public class ToBCCActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiComposeForm = event.getSource();
			UIPopupActionContainer uiActionContainer = uiComposeForm
			...
			InternetAddress[] bccAddresses = Utils
					.getInternetAddress(bccAddressString);
			List<String> emailList = new ArrayList<String>();
	...
	static public class ChangePriorityActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiForm = event.getSource();
			String priority = event.getRequestContext().getRequestParameter(
					OBJECTID);
	...
	static public class UseVisualEdiorActionListener extends
			EventListener<UIComposeForm> {
		@SuppressWarnings("deprecation")
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiForm = event.getSource();
			...
			if (isVisualEditor) {
				try {
					content = uiForm.getUIFormTextAreaInput(
							FIELD_MESSAGECONTENT).getValue();
					uiForm.removeChildById(FIELD_MESSAGECONTENT);
					UIFormWYSIWYGInput wysiwyg = new UIFormWYSIWYGInput(
							FIELD_MESSAGECONTENT, null, null, true);
				...
				try {
					content = uiForm.getChild(UIFormWYSIWYGInput.class)
							.getValue();
					uiForm.removeChild(UIFormWYSIWYGInput.class);
					UIFormTextAreaInput textArea = new UIFormTextAreaInput(
							FIELD_MESSAGECONTENT, null, null);
	...
	static public class ShowCcActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiForm = event.getSource();
			uiForm.setShowCc(!uiForm.isShowCc());
			event.getRequestContext().addUIComponentToUpdateByAjax(
					uiForm.getChildById(FIELD_TO_SET));
			event.getRequestContext().getJavascriptManager()
					.addCustomizedOnLoadScript(
							"eXo.mail.AutoComplete.init(['" + FIELD_TO + "', '"
									+ FIELD_CC + "', '" + FIELD_BCC + "']);");
		}
	}

	static public class ShowBccActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiForm = event.getSource();
	...
	static public class ReturnReceiptActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiForm = event.getSource();
	...
	private boolean validateMessage(Event<UIComposeForm> event, Message msg)
			throws Exception {
	...
	public List<ActionData> getGroupActionData(String key) {
		...
		for (String groupId : getGroupDataValues(key).keySet()) {
			...
			removeAction
					.setActionParameter(key + MailUtils.SEMICOLON + groupId);
	...
	static public class RemoveGroupActionListener extends
			EventListener<UIComposeForm> {
		public void execute(Event<UIComposeForm> event) throws Exception {
			UIComposeForm uiComposeForm = event.getSource();
			String keyAndId = event.getRequestContext().getRequestParameter(
					OBJECTID);

			try {
				uiComposeForm.getGroupDataValues(
						keyAndId.split(MailUtils.SEMICOLON)[0]).remove(
						keyAndId.split(MailUtils.SEMICOLON)[1]);
				uiComposeForm.refreshGroupFileList(keyAndId
						.split(MailUtils.SEMICOLON)[0]);
				event.getRequestContext().addUIComponentToUpdateByAjax(
						uiComposeForm.getChildById(FIELD_TO_SET));
