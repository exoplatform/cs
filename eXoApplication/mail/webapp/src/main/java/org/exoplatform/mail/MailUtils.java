...
public class MailUtils {
  ...
  public static String getAttachmentLink(Attachment att) throws RepositoryException {
    ...
    return "/" + PortalContainer.getInstance().getRestContextName() + "/private/jcr/" + repository
  ...
  public static String convertTextToHtmlLink(String s) throws Exception {
    ...
    s = decodeHTML(s);
    // for email
    s = s.replaceAll("(\\s)([_A-Za-z0-9-]+(\\.[_A-Za-z0-9-]+)*@[_A-Za-z0-9-.]+\\.[A-Za-z]{2,5})",
        "$1<a target=\"_blank\" href=\"mailto:$2\"> $2 </a>");
    ...
    while (matcher.find()) {
      ...
      tem += preffix.replace(link.substring(1), "<a target=\"_blank\" href=\"" + link.substring(1) + "\">" + link.substring(1)
          + "</a>");
