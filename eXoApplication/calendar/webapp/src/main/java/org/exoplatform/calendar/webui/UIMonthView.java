 ...
 */
@ComponentConfig(lifecycle = UIFormLifecycle.class, template = "app:/templates/calendar/webui/UIMonthView.gtmpl", events = {
...
public class UIMonthView extends UICalendarView {
  ...
  public void refresh() throws Exception {
    ...
    List<CalendarEvent> allEvents = calendarService.getEvents(username,
                                                              eventQuery,
                                                              getPublicCalendars());

    String timezone = CalendarUtils.getCurrentUserCalendarSetting().getTimeZone();
    List<CalendarEvent> originalRecurEvents = calendarService.getOriginalRecurrenceEvents(username,
    ...
    if (originalRecurEvents != null && originalRecurEvents.size() > 0) {
      ...
      while (recurEventsIter.hasNext()) {
        ...
        Map<String, CalendarEvent> tempMap = calendarService.getOccurrenceEvents(recurEvent,
                                                                                 eventQuery.getFromDate(),
                                                                                 eventQuery.getToDate(),
                                                                                 timezone);
        if (tempMap != null) {
    ...
    while (eventIter.hasNext()) {
      ...
      if (!CalendarEvent.RP_NOREPEAT.equals(event.getRepeatType())
          && !CalendarUtils.isEmpty(event.getRecurrenceId())) {
  ...
  public java.util.Calendar getEndDateOfMonthView() throws Exception {
    ...
    temCal.setFirstDayOfWeek(Integer.parseInt(calendarSetting_.getWeekStartOn()));
    temCal.add(java.util.Calendar.DATE,
               getWeeksOfTheMonth(getCurrentYear(), getCurrentMonth(), 1) * 7);
    return getBeginDay(temCal);
  }
    ...
    public void execute(Event<UIMonthView> event) throws Exception {
      ...
      try {
          ...
          if (ce != null) {
            ...
            if (ce.getCalType().equals(CalendarUtils.PRIVATE_TYPE)) {
              ...
              if (calService.getSharedCalendars(username, true) != null)
                calendar = calService.getSharedCalendars(username, true)
            ...
            if (calendar == null) {
              ...
              if ((CalendarUtils.SHARED_TYPE.equals(ce.getCalType()) && !CalendarUtils.canEdit(CalendarUtils.getOrganizationService(),
                                                                                               Utils.getEditPerUsers(calendar),
                                                                                               username))
                  || (CalendarUtils.PUBLIC_TYPE.equals(ce.getCalType()) && !CalendarUtils.canEdit(CalendarUtils.getOrganizationService(),
                                                                                                  calendar.getEditPermission(),
                                                                                                  username))) {
                continue;
              }
              CalendarEvent calEvent = ce;
              Long amount = calEvent.getToDateTime().getTime()
                  - calEvent.getFromDateTime().getTime();
        ...
        if (dataList.size() < list.size()) {
          ...
          uiApp.addMessage(new ApplicationMessage("UICalendarView.msg.some-events-cannot-move",
                                                  new Object[] { String.valueOf(number) },
                                                  1));
